...
"rho" <-
function (x = NULL, S = 35, T = 25, P = 0)
{
	# Use the same methods to retrieve STP conditions as does stp()
	stp <- stp(x, S, T, P)
	if (is.null(stp))
		stop("No 'stp' conditions provided")
	S <- stp$S
	T <- stp$T
	P <- stp$P
	...
	Ksbmw <- 19652.21 + 148.4206 * T - 2.327105 * T^2 + 1.360477e-2 * T^3 -
		5.155288e-5 * T^4

	# Secant bulk modulus of seawater at 1 atm
	Ksbm0 <- Ksbmw + S * (54.6746 - 0.603459 * T + 1.09987e-2 * T^2 -
		6.1670e-5 * T^3) + S^(3/2) * (7.944e-2 + 1.6483e-2 * T -
		5.3009e-4 * T^2)

	# Secant bulk modulus of seawater at S, T, P
	Ksbm <- Ksbm0 + P * (3.239908 + 1.43713e-3 * T + 1.16092e-4 * T^2 -
		5.77905e-7 * T^3) + P * S * (2.2838e-3 - 1.0981e-5 * T -
		1.6078e-6 * T^2) + P * S^(3/2) * 1.91075e-4 + P * P * (8.50935e-5 -
		6.12293e-6 * T + 5.2787e-8 * T^2) + P^2 * S * (-9.9348e-7 +
		2.0816e-8 * T + 9.1697e-10 * T^2)

	# Density of seawater at S, T, P
	rho <- rho0 / (1 - P / Ksbm)
	attr(rho, "unit") = "(kg/m^3)"
    return(rho)
}
